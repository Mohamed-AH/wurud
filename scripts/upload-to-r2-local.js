#!/usr/bin/env node
/**
 * Part 1b: Upload Audio Files to Cloudflare R2 (Run on Local PC)
 *
 * Uploads audio files from a local directory to Cloudflare R2.
 * Outputs a JSON manifest file for Part 2 (database update on Cloud VM).
 *
 * Usage:
 *   node scripts/upload-to-r2-local.js /path/to/audio-files [options]
 *
 * Options:
...
function getAudioFiles(directory) {
  ...
  return fs.readdirSync(directory)
    ...
    .map(file => {
      ...
      return { name: file, path: filePath, size: fileStats.size };
    });
}
...
async function uploadFile(client, bucketName, localPath, objectName) {
  ...
  const response = await client.send(new PutObjectCommand({
    ...
    ContentDisposition: getContentDisposition(objectName),
...
function getPublicUrl(publicBaseUrl, objectName) {
  const encodedName = encodeURIComponent(objectName);
  return `${publicBaseUrl}/${encodedName}`;
}

async function main() {
  console.log('\n☁️  Part 1b: Upload Audio Files to R2 (Local PC)');
  console.log('='.repeat(55));
  console.log(`Mode: ${DRY_RUN ? '🔍 DRY RUN' : '⚡ LIVE'}`);

  if (!audioDir) {
    console.log('\nUsage: node scripts/upload-to-r2-local.js <audio-directory> [options]');
    console.log('\nOptions:');
    console.log('  --env FILE        Path to .env file (default: .env)');
    console.log('  --dry-run         Show what would be uploaded');
    console.log('  --skip-existing   Skip files already in R2');
    ...
    console.log('\nExample:');
    console.log('  node scripts/upload-to-r2-local.js ./audio-files --skip-existing');
    process.exit(1);
  }
  ...
  let client, bucketName, publicBaseUrl;

  if (!DRY_RUN) {
    console.log('☁️  Initializing R2 client...');
    try {
      client = initR2Client();
      bucketName = process.env.R2_BUCKET_NAME || 'wurud-audio';
    ...
    bucketName = process.env.R2_BUCKET_NAME || 'wurud-audio';
    publicBaseUrl = process.env.R2_PUBLIC_URL || 'https://example.r2.dev';
  }
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    if (DRY_RUN) {
      const url = getPublicUrl(publicBaseUrl, file.name);
    ...
    if (SKIP_EXISTING) {
      try {
        const exists = await objectExists(client, bucketName, file.name);
        if (exists) {
          const url = getPublicUrl(publicBaseUrl, file.name);
    ...
    try {
      ...
      const result = await uploadFile(client, bucketName, file.path, file.name);
      const url = getPublicUrl(publicBaseUrl, file.name);
